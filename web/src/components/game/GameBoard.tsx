...
export function GameBoard() {
  ...
  const handleSlotSelect = (slotId: number) => {
    if (state.turnPhase !== 'selectSlot') {
      toast({ 
        title: 'Cannot select slot', 
        description: 'Select a slot at the start of your turn.', 
        variant: 'destructive' 
      });
      return;
    }
  ...
  const handleHexClick = (position: HexPosition) => {
    ...
    if (selectedTokenIndex === null) {
      toast({ 
        ...
        description: 'Click a token below first.' 
      });
      return;
    }
    placeToken(position, selectedTokenIndex);
    if (state.tokensToPlace.length > 1) {
      setSelectedTokenIndex(0);
    } else {
  ...
  const handleTakeAnimalCard = (cardId: string) => {
    if (!canTakeAnimalCard) {
      toast({ 
        title: 'Cannot take card', 
        description: 'Place all tokens first or hand is full.', 
        variant: 'destructive' 
      });
      return;
    }
  ...
  const getTurnPhaseText = () => {
    ...
    switch (state.turnPhase) {
      case 'selectSlot':
        return 'Step 1: Select a token slot';
      case 'placeTokens':
        return `Step 2: Place ${state.tokensToPlace.length} token${state.tokensToPlace.length !== 1 ? 's' : ''}`;
      case 'takeCard':
        return 'Step 3: Take an animal card (optional)';
      case 'placeCubes':
        return 'Step 4: Click habitat tokens on cards to place animals';
      default:
        return '';
    }
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-3 py-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
          {/* Left column: central board, tokens, controls */}
          ...
          <div className="lg:col-span-5 flex flex-col items-center">
            <div className="mb-2 text-center">
              ...
              <p className="text-xs text-muted-foreground">
                {isPlacingEmoji
                  ? 'Click a tile to place the animal'
                  : 'Click a hex to place token'}
              </p>
            </div>
            <HexGrid
              ...
              selectedTokenType={
                selectedTokenIndex !== null
                  ? state.tokensToPlace[selectedTokenIndex]
                  : null
              }
      ...
      {/* Global player picker for user/profile selection */}
      <PlayerPickerModal />
    </div>
  );
}
